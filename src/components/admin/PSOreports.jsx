...
function PSOreports({ year }) {
    ...
    useEffect(() => {
        getCourseApi(`staff/PSOReport?pageNo=${Active}&year=` + year, setCourseData, setTotal, setIsLoading)
    }, [Active])
    ...
    const handleInputChange = debounce(async (value) => {
        if (value.length % 2 !== 0) {
            getCourseApi(`staff/PSOReport?pageNo=&search=${value}&year=` + year, setCourseData, setTotal, setIsLoading)
        }
        else if (value.length == 0) {
            getCourseApi(`staff/PSOReport?pageNo=&year=${year}&search=`, setCourseData, setTotal, setIsLoading)
        }
    }, 500);
    //#endregion
    ...
    //#region handlePrint
    const handlePrint = () => {
        getCourseApi(`staff/PSOReportBydepartment?search=${deparment}&year=` + year, setPrintData, setTotal, setIsLoading).then(res => {
