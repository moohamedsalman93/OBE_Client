  ...
  return (
    <div className="w-full xl:h-[45rem] 2xl:h-[39rem] overflow-y-hidden space-y-2 flex flex-col items-center bg-white rounded-md shadow-md">
      <div className="flex justify-center space-x-4 my-4 ">
        <button
          className={`font-semibold px-3 py-3 ml-10 ${currentTab === "UG" ? "text-white bg-[#2d73ff] rounded-md" : " bg-slate-200 rounded-md font-semibold"}`}
          onClick={() => setCurrentTab("UG")}
        >
          UG - Arts
        </button>
        <button
          className={`font-semibold px-3 py-3 ml-10 ${currentTab === "PG" ? "text-white bg-[#2d73ff] rounded-md" : " bg-slate-200 rounded-md font-semibold"}`}
      ...
      {isLoading ? (
        ...
        <div className="w-full space-y-4 flex flex-col items-center overflow-y-auto">
          ...
          <div className="mt-4 w-full font-medium text-[#3b84f4] py-3 text-center flex justify-center">
            <BarChart
              ...
              data={filteredData.map((item) => ({
                name: item.depTitle,
                value: parseFloat(item.overAtain).toFixed(2),
              }))}
              margin={{
                top: 5,
                right: 30,
